...
namespace AmplifyShaderEditor
{
	...
	public class WeightedAvgNode : ParentNode
	{
		...
		protected void GetInputData( WirePortDataType inputPortType, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			m_inputData[ 0 ] = m_inputPorts[ 0 ].GenerateShaderForOutput( ref dataCollector, inputPortType, ignoreLocalvar );
			for ( int i = 1; i < m_inputPorts.Count; i++ )
			{
				if ( m_inputPorts[ i ].Visible )
				{
					m_inputData[ i ] = m_inputPorts[ i ].GenerateShaderForOutput( ref dataCollector, m_mainDataType, ignoreLocalvar, 0, true );
				}
			}
		}
	}
