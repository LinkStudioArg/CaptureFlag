...
namespace AmplifyShaderEditor
{
	...
	public sealed class SimpleContrastOpNode : ParentNode
	{
		...
		{
			string contrastValue = m_inputPorts[ 0 ].IsConnected ? m_inputPorts[ 0 ].GenerateShaderForOutput( ref dataCollector, inputPortType, ignoreLocalVar ) : m_defaultContrast.ToString();
			string colorTarget = m_inputPorts[ 1 ].GenerateShaderForOutput( ref dataCollector, inputPortType, ignoreLocalVar );

			WirePortDataType connType = m_inputPorts[ 1 ].ConnectionType();
			if ( connType != WirePortDataType.COLOR || connType != WirePortDataType.FLOAT4 )
			{
				colorTarget = UIUtils.CastPortType( m_currentPrecisionType, new NodeCastInfo( m_uniqueId, outputId ), colorTarget, connType, WirePortDataType.COLOR, colorTarget );
			}
